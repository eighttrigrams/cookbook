...
(def bound-as
  ...
  **Three tables share one binding, and that is the schema being told the truth
  ...
  into a useful-when, a reason into a context, a Scope's prose into a Recipe's."
  {:recipes :recipe
...
(defn seal
  ...
  `stored` comes back byte for byte, so the server's value comparison still sees
  'unchanged'. Anything genuinely new gets a fresh nonce, so two Recipes saying
  the same sentence still seal differently.

  A `stored` that will not open is treated as *not matching* rather than as an
  error: refusing the write would leave the reader holding the only copy of the
  new text with nowhere to put it."
  ...
  ([k table column v stored]
   (cond
     ...
     (sealed? stored)
     (.then (unseal k table column stored)
